...
item_images = { "Gem": load_image('assets/images/items/platformPack_item008.png'),
                "SpeedPowerUp": load_image('assets/images/items/platformPack_item009.png')}
...
class Hero(pygame.sprite.Sprite):
    ...
    def can_jump(self, tiles):
        ...
        self.rect.x += 2
        hit_list2 = pygame.sprite.spritecollide(self, tiles, False)
        self.rect.x -= 2

        self.rect.x -= 2
        hit_list3 = pygame.sprite.spritecollide(self, tiles, False)
        self.rect.x += 2

        return len(hit_list) > 0 or len(hit_list2) > 0 or len(hit_list3) > 0
...
class BouncingEnemy(BasicEnemy):
    '''
    BouncingEnemies move back and forth, turning around whenever
    they hit a block or the edge of the world. Gravity affects
    BasicEnemies, so they will walk off platforms.
    '''
...
class Level():
    ...
    def load_items(self):
        ...
        for element in self.map_data['items']:
            ...
            elif kind == "SpeedPowerUp":
                s = SpeedPowerUp(x, y, item_images[kind])
    ...
    def load_enemies(self):
        ...
        for element in self.map_data['enemies']:
            ...
            elif kind == "BouncingEnemy":
                s = BouncingEnemy(x, y, basic_enemy_images)
